    ...
    for (let i = 0; i < masStr.length; i++) {
      ...
      resStr.push(
        <Grid key={i} container sx={{ marginBottom: 0 }}>
          ...
          {TablStr(0, 3, masStr[i].period, styleBl1Form09(brb))}
          {TablStr(0, 3, masStr[i].why, styleBl1Form099(brb))}
          {TablStr(0, 3, masStr[i].status, styleBl1Form09(brb))}
          {TablStr(0, 0, quo, styleBl1Form09(brb))}
        </Grid>
      );
    }
    return resStr;
  ...
  const HeaderTabl = () => {
    return (
      <Grid container sx={styleBl1Form07}>
        ...
        {TablStr(1, 3, "Период", styleBl1Form08)}
        {TablStr(1, 3, "Причина", styleBl1Form08)}
        {TablStr(1, 3, "Статус", styleBl1Form08)}
        {TablStr(1, 0, "", styleBl1Form08)}
      </Grid>
    );
  };
